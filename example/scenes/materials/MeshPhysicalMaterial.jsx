...
export default () => {
	...
	const [ materialProps, setMaterialProps ] = useState({
		...
		values: {
			...
			clearcoat: 0.5, // Float, 0 - 1
			clearcoatRoughness: 0.5, // Float 0 - 1
			ior : 1.5,  // Float, 1 - 2.333
			reflectivity: 0.5,  // Float 0 - 1
			sheen: 0.5, // Float 0 - 1
			sheenRoughness: 0.5, // Float 0 - 1
			transmission: 0.5,
			transparent: false,
			opacity: 0.95,
			alphaTest: 0.5, //   0 - 1
	...
	return <>
		...
		<div className="info">
			...
			sheenColor: 
				<input type="color" value={materialProps.colors.sheenColor}
					onChange={ ({target: {value}}) => {	setMaterialProps({...materialProps, colors: {
						...materialProps.colors,
						sheenColor: value
					}}); } }
				/> <span className="show-value">[{materialProps.colors.sheenColor}]</span><br />
			...
			clearcoat: 
				<input value={materialProps.values.clearcoat} type="range" min="0" max="1" step="0.01"
					onChange={ ({target: {value}}) => {	setMaterialProps({...materialProps, values: {
						...materialProps.values,
						clearcoat: parseFloat(value)
					}}); } }
				/> <span className="show-value">[{materialProps.values.clearcoat}]</span><br />

			clearcoatNormalScale X:
				<input value={materialProps.vectors2.clearcoatNormalScale[0]} type="range" min="0" max="20" step="0.02"
					onChange={ ({target: {value}}) => {	setMaterialProps({...materialProps, vectors2: {
						...materialProps.vectors2,
						clearcoatNormalScale: [parseFloat(value), materialProps.vectors2.clearcoatNormalScale[1]]
					}}); } }
				/> <span className="show-value">[{materialProps.vectors2.clearcoatNormalScale[0]}]</span><br />

			clearcoatNormalScale Y:
				<input value={materialProps.vectors2.clearcoatNormalScale[1]} type="range" min="0" max="20" step="0.02"
					onChange={ ({target: {value}}) => {	setMaterialProps({...materialProps, vectors2: {
						...materialProps.vectors2,
						clearcoatNormalScale: [materialProps.vectors2.clearcoatNormalScale[0], parseFloat(value)]
					}}); } }
				/> <span className="show-value">[{materialProps.vectors2.clearcoatNormalScale[1]}]</span><br />

			clearcoatRoughness: 
				<input value={materialProps.values.clearcoatRoughness} type="range" min="0" max="1" step="0.01"
					onChange={ ({target: {value}}) => {	setMaterialProps({...materialProps, values: {
						...materialProps.values,
						clearcoatRoughness: parseFloat(value)
					}}); } }
				/> <span className="show-value">[{materialProps.values.clearcoatRoughness}]</span><br />

			ior: 
				<input value={materialProps.values.ior} type="range" min="1" max="2.33" step="0.01"
					onChange={ ({target: {value}}) => {	setMaterialProps({...materialProps, values: {
						...materialProps.values,
						ior: parseFloat(value)
					}}); } }
				/> <span className="show-value">[{materialProps.values.ior}]</span><br />
			...
			sheen: 
				<input value={materialProps.values.sheen} type="range" min="0" max="1" step="0.01"
					onChange={ ({target: {value}}) => {	setMaterialProps({...materialProps, values: {
						...materialProps.values,
						sheen: parseFloat(value)
					}}); } }
				/> <span className="show-value">[{materialProps.values.sheen}]</span><br />
			
			sheenRoughness: 
				<input value={materialProps.values.sheenRoughness} type="range" min="0" max="1" step="0.01"
					onChange={ ({target: {value}}) => {	setMaterialProps({...materialProps, values: {
						...materialProps.values,
						sheenRoughness: parseFloat(value)
					}}); } }
				/> <span className="show-value">[{materialProps.values.sheenRoughness}]</span><br />

			transmission: 
				<input value={materialProps.values.transmission} type="range" min="0" max="1" step="0.01"
					onChange={ ({target: {value}}) => {	setMaterialProps({...materialProps, values: {
						...materialProps.values,
						transmission: parseFloat(value)
					}}); } }
				/> <span className="show-value">[{materialProps.values.transmission}]</span><br />
		...
		<div className="links-block">
			<a href="https://github.com/shstefanov/orbits-engine-examples/blob/master/example/scenes/materials/MeshPhysicalMaterial.jsx"> Code </a>
		</div>
